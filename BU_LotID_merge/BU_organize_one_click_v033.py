...
GENERATED_DIR_SUFFIXES = ("_LotID_latest_v1", "_LotID_latest_v1_cropped_v1")
WRITE_CROP_REPORT_DETAIL_SHEET = True
WRITE_CROP_REPORT_VISUALIZATION_SHEET = True
WRITE_BU_ANALYSIS_EXCEL = True
...
def normalize_lot_id(value) -> str:
...
def safe_remove_generated_dir(path: Path) -> None:
    ...
    if not any(resolved.name.endswith(suffix) for suffix in GENERATED_DIR_SUFFIXES):
        raise RuntimeError(f"자동 생성 결과 폴더가 아니어서 삭제하지 않습니다: {resolved}")
    if resolved.exists():
...
def parse_lmk_time(value: str, fallback_path: Path | None = None) -> datetime:
...
def collect_latest_measurements(data_root: Path, cancel_check=None) -> tuple[dict[str, dict], list[dict]]:
    ...
    for file_idx, csv_path in enumerate(csv_files, start=1):
        ...
        for row in iter_csv_dict_rows(csv_path):
            ensure_not_cancelled(cancel_check)
            lot_id = first_row_value(row, LOT_ID_CANDIDATES)
            if not lot_id:
                continue

            measured_at_raw = row.get("Time", "")
            measured_at = parse_lmk_time(measured_at_raw, csv_path)
            record = {
                "lot_id": lot_id,
            ...
            current = latest_by_lotid.get(normalized_lot_id)
            is_latest = current is None or measured_at >= current["time_obj"]
            if is_latest:
                latest_by_lotid[normalized_lot_id] = record
...
def pick_worst_lotids(latest_measurements: dict[str, dict], key: str, limit: int = 10) -> list[tuple[str, str, float]]:
    ...
    for lot_id, measurement in latest_measurements.items():
        ...
        if value is None:
            continue
        rows.append((measurement.get("lot_id", lot_id), measurement.get("judge", ""), value))
...
def copy_latest_folders(latest_by_lotid: dict[str, Path], output_root: Path, cancel_check=None) -> None:
    ...
    if output_root.exists():
        safe_remove_generated_dir(output_root)
...
def write_bu_analysis_excel(
    ...
    for idx, rec in enumerate(sorted(bu_records, key=lambda item: item["lot_id"]), start=1):
        ensure_not_cancelled(cancel_check)
        lot_id = rec["lot_id"]
        measurement = find_measurement(lot_id, latest_measurements)
...
def crop_images(input_root: Path, output_root: Path, threshold: int, padding: int, cancel_check=None):
    ...
    def process_one(item):
        ...
        with progress_lock:
            processed_count += 1
            if processed_count == 1 or processed_count % 10 == 0 or processed_count == total_images:
                print_progress("  크롭 진행", processed_count, total_images, done=(processed_count == total_images))

        return record

    max_workers = min(32, (os.cpu_count() or 4) + 4)
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        records = list(executor.map(process_one, enumerate(image_files, start=1)))
...
def write_excel(
    ...
    for lot_id in sorted(grouped.keys()):
        ...
        measurement = find_measurement(lot_id, latest_measurements or {})
    ...
    if WRITE_CROP_REPORT_DETAIL_SHEET:
    ...
    if WRITE_CROP_REPORT_VISUALIZATION_SHEET:
