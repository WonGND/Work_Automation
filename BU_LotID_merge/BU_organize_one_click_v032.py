...
def style_line_chart(chart: LineChart, value_color: str, spec_color: str) -> None:
    ...
    if len(chart.ser) >= 1:
        ...
        chart.ser[0].graphicalProperties.line.width = 26000
    ...
    if len(chart.ser) >= 2:
        ...
        chart.ser[1].graphicalProperties.line.width = 16000
...
def add_visualization_sheet(wb: Workbook, latest_measurements: dict[str, dict]) -> None:
    ...
    ws["A1"] = "BU / WU Quality Dashboard"
    ws["A1"].font = Font(size=20, bold=True, color="FFFFFF")
    ws["A1"].fill = PatternFill("solid", fgColor="0F172A")
    ws["A1"].alignment = Alignment(horizontal="left", vertical="center")
    ws["A3"] = "BU와 WU를 같은 행에 배치해 분포 추세, 분포 구간, Pass/Fail을 한눈에 비교"
    ws["A3"].font = Font(size=10, color="64748B")
    ws["Q3"] = datetime.now().strftime("Updated: %Y-%m-%d %H:%M:%S")
    ws["Q3"].font = Font(size=10, color="64748B")
    ws["Q3"].alignment = Alignment(horizontal="right")
    ...
    write_card(ws, "A5", "전체 LotID", total_count, "최신 측정 기준", "0F766E")
    write_card(ws, "D5", "BU Fail", bu_summary["fail_count"], f"Spec {BU_SPEC_MIN}", "B91C1C")
    write_card(ws, "G5", "WU Fail", wu_summary["fail_count"], f"Spec {WU_SPEC_MIN}", "C2410C")
    write_card(ws, "J5", "Pass Rate", f"{pass_rate:.1f}%", "BU+WU combined", "1D4ED8")

    ws["A9"] = "Metric Summary"
    ws["A9"].font = Font(size=13, bold=True, color="111827")
    ws["A10"] = "지표"
    ws["B10"] = "Spec Min"
    ws["C10"] = "Count"
    ws["D10"] = "Pass"
    ...
    for idx, value in enumerate(bu_summary["sorted_values"], start=3):
        ws.cell(row=idx, column=21, value=idx - 2)
        ws.cell(row=idx, column=22, value=value)
        ws.cell(row=idx, column=23, value=BU_SPEC_MIN)

    ws["X2"] = "WU_index"
    ws["Y2"] = "WU_value"
    ws["Z2"] = "WU_spec"
    for idx, value in enumerate(wu_summary["sorted_values"], start=3):
        ws.cell(row=idx, column=24, value=idx - 2)
        ws.cell(row=idx, column=25, value=value)
        ws.cell(row=idx, column=26, value=WU_SPEC_MIN)

    bu_line = LineChart()
    bu_line.title = "BU 분포 추세"
    bu_line.y_axis.title = "Black Uniformity"
    bu_line.x_axis.title = "정렬 순서"
    bu_data = Reference(ws, min_col=22, max_col=23, min_row=2, max_row=max(3, len(bu_summary["sorted_values"]) + 2))
    bu_cats = Reference(ws, min_col=21, min_row=3, max_row=max(3, len(bu_summary["sorted_values"]) + 2))
    bu_line.add_data(bu_data, titles_from_data=True)
    bu_line.set_categories(bu_cats)
    style_line_chart(bu_line, "DC2626", "94A3B8")
    ws.add_chart(bu_line, "A15")

    wu_line = LineChart()
    wu_line.title = "WU 분포 추세"
    wu_line.y_axis.title = "White Uniformity"
    wu_line.x_axis.title = "정렬 순서"
    wu_data = Reference(ws, min_col=25, max_col=26, min_row=2, max_row=max(3, len(wu_summary["sorted_values"]) + 2))
    wu_cats = Reference(ws, min_col=24, min_row=3, max_row=max(3, len(wu_summary["sorted_values"]) + 2))
    wu_line.add_data(wu_data, titles_from_data=True)
    wu_line.set_categories(wu_cats)
    style_line_chart(wu_line, "16A34A", "94A3B8")
    ws.add_chart(wu_line, "J15")
    ...
    ws["AD2"] = "Fail"
    ws["AB3"] = "BU"
    ws["AC3"] = bu_summary["pass_count"]
    ws["AD3"] = bu_summary["fail_count"]
    ws["AB4"] = "WU"
    ws["AC4"] = wu_summary["pass_count"]
    ws["AD4"] = wu_summary["fail_count"]

    bu_pie = PieChart()
    bu_pie.title = "BU Pass/Fail"
    bu_pie.add_data(Reference(ws, min_col=29, max_col=30, min_row=3, max_row=3), from_rows=True)
    bu_pie.set_categories(Reference(ws, min_col=29, max_col=30, min_row=2, max_row=2))
    bu_pie.dataLabels = DataLabelList()
    bu_pie.dataLabels.showPercent = True
    bu_pie.dataLabels.showVal = True
    style_pie_chart(bu_pie)
    ws.add_chart(bu_pie, "A40")

    wu_pie = PieChart()
    wu_pie.title = "WU Pass/Fail"
    wu_pie.add_data(Reference(ws, min_col=29, max_col=30, min_row=4, max_row=4), from_rows=True)
    wu_pie.set_categories(Reference(ws, min_col=29, max_col=30, min_row=2, max_row=2))
    wu_pie.dataLabels = DataLabelList()
    wu_pie.dataLabels.showPercent = True
    wu_pie.dataLabels.showVal = True
    style_pie_chart(wu_pie)
    ws.add_chart(wu_pie, "J40")
    ...
    for idx, (label, count) in enumerate(bu_distribution, start=3):
        ws.cell(row=idx, column=32, value=label)
        ws.cell(row=idx, column=33, value=count)

    ws["AI2"] = "WU_bucket"
    ws["AJ2"] = "WU_count"
    for idx, (label, count) in enumerate(wu_distribution, start=3):
        ws.cell(row=idx, column=35, value=label)
        ws.cell(row=idx, column=36, value=count)

    bu_bar = BarChart()
    bu_bar.title = "BU 분포 구간"
    bu_bar.y_axis.title = "Count"
    bu_bar.x_axis.title = "Range"
    bu_bar.add_data(Reference(ws, min_col=33, min_row=2, max_row=2 + len(bu_distribution)), titles_from_data=True)
    bu_bar.set_categories(Reference(ws, min_col=32, min_row=3, max_row=2 + len(bu_distribution)))
    style_bar_chart(bu_bar, "DC2626")
    ws.add_chart(bu_bar, "A27")

    wu_bar = BarChart()
    wu_bar.title = "WU 분포 구간"
    wu_bar.y_axis.title = "Count"
    wu_bar.x_axis.title = "Range"
    wu_bar.add_data(Reference(ws, min_col=36, min_row=2, max_row=2 + len(wu_distribution)), titles_from_data=True)
    wu_bar.set_categories(Reference(ws, min_col=35, min_row=3, max_row=2 + len(wu_distribution)))
    style_bar_chart(wu_bar, "16A34A")
    ws.add_chart(wu_bar, "J27")

    ws["A14"] = "분포 추세"
    ws["A14"].font = Font(size=12, bold=True, color="1F2937")
    ws["A26"] = "분포 구간"
    ws["A26"].font = Font(size=12, bold=True, color="1F2937")
    ws["A39"] = "Pass / Fail"
    ws["A39"].font = Font(size=12, bold=True, color="1F2937")

    write_kpi_table(ws, 48, 1, "Worst BU LotID", [(lot_id, value) for lot_id, judge, value in pick_worst_lotids(latest_measurements, "black_uniformity", 5)])
    write_kpi_table(ws, 48, 5, "Worst WU LotID", [(lot_id, value) for lot_id, judge, value in pick_worst_lotids(latest_measurements, "white_uniformity", 5)])
    ...
    for hidden_col in ("T", "U", "V", "X", "Y", "Z", "AB", "AC", "AD", "AF", "AG", "AI", "AJ"):
