...
class BUOrganizeApp:
    ...
    def _on_success(self, result: dict) -> None:
        ...
        self.main_result_var.set(Path(result.get("excel_path", "")).name if result.get("excel_path") else "생성되지 않음")
    ...
    def _handle_auto_open(self) -> None:
        if not self.latest_result:
            return
        auto_open = self.auto_open_var.get()
        if auto_open == "excel" and self.latest_result.get("excel_path"):
            self._open_result(self.latest_result["excel_path"])
        elif auto_open == "folder" and self.latest_result.get("excel_path"):
            self._open_path(Path(self.latest_result["excel_path"]).parent)
        elif auto_open == "both" and self.latest_result.get("excel_path"):
            self._open_result(self.latest_result["excel_path"])
            self._open_path(Path(self.latest_result["excel_path"]).parent)
